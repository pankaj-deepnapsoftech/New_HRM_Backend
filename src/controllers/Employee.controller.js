...
export const ListEmployeesWithPagination = AsyncHandler(async (req, res) => {
    ...
    // UPDATED: Fetch from EmpData (primary) and populate verificationDetails for linked data
    const [employees, total] = await Promise.all([
        EmpData.find()
            .populate({
                path: "verificationDetails",  // Populate linked Employee data (Aadhaar, bank, etc.)
                select: "aadhaar pancard photo Bank_Proof Voter_Id Driving_Licance UAN_number Bank_Account Bank_Name IFSC_Code"  // Select specific fields
            })
            .skip(skip)
            .limit(limit)
            .sort({ createdAt: -1 }),
        EmpData.countDocuments()
    ]);
