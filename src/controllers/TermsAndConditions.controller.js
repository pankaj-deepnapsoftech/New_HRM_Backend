...
export const submitTermsAndConditions = AsyncHandler(async (req, res) => {
  ...
  const rawCandidates = [employeeId, authUser.employeeId, authUser.email, authUser._id]
    .filter(Boolean)
    .map(String)
  ...
  const employee = await EmpData.findOne({ $or: queries.length ? queries : [{ _id: new mongoose.Types.ObjectId('000000000000000000000000') }] });
  
  if (!employee) {
    throw new NotFoundError("Employee not found", "submitTermsAndConditions");
  }

  // Prevent duplicate submissions
  if (employee.termsAndConditions?.submitted) {
    throw new ConflictError("Terms and conditions already submitted", "submitTermsAndConditions");
  }
  ...
  employee.termsAndConditions = {
    ...
    ipAddress,
    userAgent,
    agreementText: agreementText || "I agree to all the terms and conditions"
  };
...
export const getEmployeeTermsStatus = AsyncHandler(async (req, res) => {
  ...
  const employee = await EmpData.findOne({ empCode: employeeId }, {
    fname: 1,
    lastName: 1,
    email: 1,
    empCode: 1,
...
export const sendTermsReminder = AsyncHandler(async (req, res) => {
  ...
  const employee = await EmpData.findOne({ $or: orQuery.length ? orQuery : [{ _id: new mongoose.Types.ObjectId('000000000000000000000000') }] });
...
export const getTermsStatistics = AsyncHandler(async (req, res) => {
  ...
  res.status(200).json({
    ...
    data: {
      ...
      submissionRate: totalEmployees > 0 ? ((submittedCount / totalEmployees) * 100).toFixed(2) : 0
    }
  });
});
