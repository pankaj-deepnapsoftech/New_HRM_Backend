...
export const applyLeaveToFinal = async (requestDoc, session = null) => {
    ...
    while (current <= toDate) {
        ...
        const daysThisMonth =
            Math.floor((lastDayThisMonth - current) / msPerDay) + 1;
    ...
    try {
        ...
        if (!employeeId || !from || !to || !type) {
            return res.status(400).json({
                success: false,
                message: 'employeeId, from, to and type required',
            });
        }
...
export const getEmployeeLeaves = async (req, res) => {
    try {
        ...
        const docs = await Leave.find({ employeeId, year: Number(year) }).sort({
            month: 1,
        });
...
/**
 * Get employee leave requests for a year (with status)
 */
